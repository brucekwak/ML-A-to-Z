...
# Random forest
# [Reference] https://github.com/llSourcell/random_forests/blob/master/Random%20Forests%20.ipynb
...
############################################
# Decision Tree Algorithm helper function 
############################################
...
def evaluate_algorithm(dataset, algorithm, n_folds, *args):
	...
	folds = cross_validation_split(dataset, n_folds)  # split the dataset in k fold
	scores = list()
...
def gini_index(groups, class_values):
	...
	for class_value in class_values:
		...
		for group in groups:
			...
			gini += (proportion * (1.0 - proportion))   # former proportion -> weight  (we want weighted sum of gini index)
	return gini   # weighted sum of gini index
...
def get_split(dataset, n_features):
	...
	b_index, b_value, b_score, b_groups = 999, 999, 999, None   # really big number
	features = list()
...
filename = 'german_credit_data.data-numeric'
dataset = load_csv(filename)

# Data preprocessing
dataset_after_preprocessing = []
for row_i in dataset:
    temp = row_i[0]
    temp_2 = temp.split()
    temp_3 = list(map(int, temp_2))
    dataset_after_preprocessing.append(temp_3)
    
# get class_info
class_info = list(set(row[-1] for row in dataset_after_preprocessing))
print(class_info)



# evaluate algorithm
n_folds = 5   # fold = subsamples of data
max_depth = 10   # how many level of tree we want
min_size = 1    # minimum size
sample_size = 1.0
n_features = int(sqrt(len(dataset_after_preprocessing[0])-1))   # 5
#n_features = 7

# making 3 random forest model and compare each other
for n_trees in [1, 5, 10]:   # 1, 5, 10 tree
	scores = evaluate_algorithm(dataset_after_preprocessing, random_forest, n_folds, max_depth, min_size, sample_size, n_trees, n_features)
